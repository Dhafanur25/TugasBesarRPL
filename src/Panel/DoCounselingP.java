...
public class DoCounselingP extends javax.swing.JPanel {
    ...
    /**
     * Creates new form DoCounselingP
     */
    ...
    private void boxKonselorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_boxKonselorActionPerformed
        ...
        String nama = db.getDef();
        String email = db.getDef();
        String phone = db.getDef();
        String exp = db.getDefExp();
        ...
        if (!numericPart.equals("<Choose Counselor>")){
            ...
            ResultSet account = db.getDB("konselor", "ID", numericPart);
            try {
                if (account.next()){
        ...
        ResultSet account = db.getHariDB(IDKon);
        try {
            while(account.next()){
    ...
    public void doSetSchedule(){
        ...
        int IDHari = 0;
        int IDJam = 0;       
        ResultSet account = db.getDB("hari_konseling", "Hari", isiH);
        try {
            if (account.next()){
        ...
        account = db.getDB("waktu_konseling", "Waktu", isiJ);
        try {
            if (account.next()){
        ...
        if (db.insertJadDB(IDKon, IDHari, IDJam)){
    ...
    private void boxDayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_boxDayActionPerformed
        ...
        if (!isiH.equals("<Choose Counseling Day>")){
            ...
            ResultSet account = db.getWaktuDB(IDKon, isiH);
            try {
                while(account.next()){
